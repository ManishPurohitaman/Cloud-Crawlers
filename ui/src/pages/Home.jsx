...
export default class Home extends React.Component {
  ...
  gettableData= (cpu,ram) =>  {
    console.log(`http://0.0.0.0:8000/api/v1/vm/sizer/${cpu}/${ram}`)
    fetch(`http://0.0.0.0:8000/api/v1/vm/sizer/${cpu}/${ram}`)
      .then((res) => res.json())
      .then(
        (data) => {
  ...
  displayTable() {
    ...
    if (this.state.showTable){
      var ram=this.state.memory
      var cpu=this.state.vcpu
    if(ram!='' && cpu!='')
    this.gettableData(cpu,ram)
  ...
  render() {
    ...
    const leftPanel = (
      ...
      >
        <Link to={`/home`}>
        <MenuItem key={"/azure"}>Instance Info</MenuItem>
        </Link>
        <Link to={`/home2`}>
        <MenuItem key={"/quotar"}>Instance Matching</MenuItem>
        </Link>
      </Menu>
    );    
    const sort = {
    ...
    const body = (
      <div>
        <div className="heading">
          <h1 style={{ textAlign:"center", marginTop: 25, marginBottom:25}}>X Cloud Workload Analyzer - 1</h1>
        </div>
          ...
          >
            ...
            <FormSection title="General Configuration">
              <FormItemInput
                id="vcpu"
                label="vCPUs"
                value={ this.state.vcpu }
                onChange={ (event) => this.setState({ vcpu: event.target.value }) }
                 />
              <FormItemInput
                id="memory"
                label="Memory(GB)"
